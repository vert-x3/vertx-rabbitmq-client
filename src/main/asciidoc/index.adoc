...
It is also possible for the delivery tag of a single message to change (delivery tags are per-channel, so the message is resent following a reconnection it will have a new delivery tag) - this means that we cannot use a Future to inform the client of the delivery tag.
